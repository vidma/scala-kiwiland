 ...
 *
 *  just recursively walk through all possible paths and counting the ones that matches the
 *  required conditions. The conditions are specified as two guard functions:
 *  - searchGuard(stops_used_so_far, dist_so_far) = shall we continue search deeper?
 *  - reachedGuard(stops_used_so_far, dist_so_far) = is some path using that many stops
 *        or being at the given distance acceptable as a solution?
 * */
